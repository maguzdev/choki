...
function Editor({ categories, product, onClose }: { categories: Category[]; product?: Product; onClose: () => void }) {
  ...
  return <form action={action} className="rounded-lg border border-cream-200 bg-cream-50 p-4">
    ...
    <div className="mt-4 grid gap-3 sm:grid-cols-2">
      ...
      <label className="text-sm font-semibold">Costo de referencia<input name="cost" type="number" min="0" inputMode="numeric" defaultValue={product?.cost ?? 0} className="mt-1 h-11 w-full rounded-lg border border-cream-200 bg-white px-3" required /></label>
    ...
    {product ? <p className="mt-4 rounded-lg bg-cream-100 p-3 text-sm text-choco-600">Costo promedio actual: <strong>{product.avg_cost}</strong>. Se calcula en compras y no se edita aquí.</p> : null}
